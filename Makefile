...
# --specs nano.specs: This flag tells the linker to use a specific set of options that create a **smaller executable**. The `nano.specs` option is commonly used for embedded systems to minimize the size of the output. (Minimal version of newlib->newlib-nano)
#
# -lc: This tells the linker to include the C standard library (`libc/newlib`), even though we’re avoiding the standard library by using `-nostdlib`. This is necessary to handle basic C runtime features, like memory allocation.
# In embedded ARM toolchains like GCC for ARM (used in your project), newlib is the default implementation of the C standard library, not glibc (which is commonly used for desktop systems)
#
